...
const consonantCases = [
    ["က", "ker"],  ["ခ", "ker"],  ["ဂ", "ger"],  ["ဃ", "kher"], ["င", "nger"],
    ["စ", "ser"],  ["ဆ", "ser"],  ["ၡ", "sher"], ["ည", "nyer"], ["တ", "ter"],
    ["ထ", "ter"],  ["ဒ", "der"],  ["န", "ner"],  ["ပ", "ber"],  ["ဖ", "per"],
    ["ဘ", "ber"],  ["မ", "mer"],  ["ယ", "yer"],  ["ရ", "rer"],  ["လ", "ler"],
    ["ဝ", "wer"],  ["သ", "ther"], ["ဟ", "her"],  ["အ", "a"],    ["ဧ", "haer"],
];
...
// ─────────────────────────────────────────────────────────────────────────────
// 7. ALL 5 MEDIALS — each tested with a consonant that actually takes it
//    per the sgaw-complete-syllable-list.txt reference.
//    ြ (r): တ takes it   ျ (l): ပ takes it   ွ (w): တ takes it
//    ှ (w): ပ takes it   ၠ (j): ပ takes it
// ─────────────────────────────────────────────────────────────────────────────
section("All 5 medials (valid consonant bases)");

assert("တြါ (t + ြ→r + a)  → tra", transliterate("တြါ"), "tra");
assert("ပျါ (b + ျ→l + a)  → bla", transliterate("ပျါ"), "bla");
assert("တွါ (t + ွ→w + a)  → twa", transliterate("တွါ"), "twa");
assert("ပှါ (b + ှ→w + a)  → bwa", transliterate("ပှါ"), "bwa");
// ၠ sound depends on consonant: က+ၠ→j, ခ+ၠ→ch; other combos (ပ/ဖ/ဘ/မ+ၠ) are rare/not tested
assert("ကၠါ (k + ၠ→j + a)  → ja",  transliterate("ကၠါ"), "ja");
assert("ခၠါ (k + ၠ→ch + a) → cha", transliterate("ခၠါ"), "cha");

// ─────────────────────────────────────────────────────────────────────────────
// CONSONANT CLUSTERS — ခ+ရ acts as a single "kr" unit
// ─────────────────────────────────────────────────────────────────────────────
section("Consonant clusters");

assert("ခရ    (ခ+ရ cluster → kr + er) → krer", transliterate("ခရ"),    "krer");
assert("ခရၢ်  (kr + ah)               → krah", transliterate("ခရၢ်"),  "krah");
assert("ခရံာ် (kr + ee)               → kree", transliterate("ခရံာ်"), "kree");
assert("ခ alone still → ker",                   transliterate("ခ"),      "ker");
assert("ရ alone still → rer",                   transliterate("ရ"),      "rer");
...
assert("ပှါ   (b + w + a)        → bwa",   transliterate("ပှါ"),  "bwa");
...
assert("လုၢ် (l + u)             → lu",   transliterate("လုၢ်"), "lu");
assert("လီၢ် (l + aw)           → law",  transliterate("လီၢ်"), "law");
...
assert(
    ...
    "tah lu law, ther sho a lee saw see a saw naw koh, bwa hee po khaw po a tah ba."
);

// ─────────────────────────────────────────────────────────────────────────────
// Hymn / multi-line song verse
// ─────────────────────────────────────────────────────────────────────────────
section("Hymn verse (4 lines)");

assert(
    "hymn line 1",
    transliterate("ပစံးဘျုးယွၤလၢဟ့ၣ်ပှၤမုၢ်နံၤအဃံလၤ"),
    "ber see blu ywa ler hay bwa mu nee a khee la"
);
assert(
    "hymn line 2",
...
assert(
    "hymn line 4",
    transliterate("မ့ၢ်တၢ်ဆိၣ်ဂ့ၤအဒိၣ်ကတၢၢ်ပဂီၢ်"),
    "may tah soh gay a doh ker ter ber gaw"
);
